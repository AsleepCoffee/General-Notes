...
# Exersie - config backend pools for encryption
...
Deploy a virtual machine and an application gateway
    ...
    Open the Azure Cloud Shell in your browser, and log in to the directory with access to the subscription you want to create resources in.
...
In Azure Cloud Shell, run the following command to download the source code for the shipping portal.
Bash

git clone https://github.com/MicrosoftDocs/mslearn-end-to-end-encryption-with-app-gateway shippingportal

Move to the shippingportal folder.
Bash

cd shippingportal

Run the following setup script to create the virtual machine, certificates, and application gateway.
Bash
...
Verify that the web server is configured correctly
    ...
    Run the following command to display the URL of the web server that the setup script created.
    Bash
...
Configure the backend pool for encryption
    ...
    Run the following command to get the private IP address of the virtual machine that's acting as the web server.
    Bash
...
Upload the certificate for the VM in the backend pool to Application Gateway, as a trusted root certificate. This certificate was generated by the setup script and is stored in the shipping-ssl.crt file.
Azure CLI
...
Upload the SSL certificate for Application Gateway. The setup script generated this certificate in the previous exercise. The certificate is stored in the appgateway.pfx file in the server-config folder.
